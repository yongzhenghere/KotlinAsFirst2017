...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    ...
    val article = File(inputName).readText().toLowerCase()
    for (element in substrings) {
        result.put(element, article.split(element.toLowerCase()).size - 1)
    }
    return result
}
...
fun sibilants(inputName: String, outputName: String) {
    ...
    for (element in File(inputName).readText()) {
        if (result in letters && map.containsKey(element))
            writer.append(map[element] ?: ' ')
        else writer.append(element)
        result = element
    }
    writer.close()
}
...
fun centerFile(inputName: String, outputName: String) {
    ...
    for (i in 0 until lines.size) {
        val lineLen = lines[i].trim().length
        for (j in 1..(maxLen - lineLen) / 2) writer.write(" ")
